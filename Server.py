...
def process_player_move(player_move: PlayerState.PlayerMovement, client_addr: str, gamestate: PlayerState.GameState):
    ...
    if player_move.keys[str(arcade.key.A)]:
        delta_x = -3
        delta_y = -3
    else:
...
def check_if_at_target(player: PlayerState.PlayerState, target: PlayerState.TargetState,
    ...
    player.points += 5
    gamestate.target = PlayerState.TargetState(random.randint(36, PlayerState.WINDOW_WIDTH - 36),
